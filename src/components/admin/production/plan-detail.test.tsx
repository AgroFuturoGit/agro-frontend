...
// Nenhum acesso de rede real: os 3 GET da tela são mockados, então
// `apiRequest` nunca é alcançado (mesmo padrão de
// `production-plans-page.test.tsx`). O resto do módulo é preservado via
...
describe("PlanDetail — gating das ações de escrita por role", () => {
  it.each(["MANAGER", "TECHNICIAN", "ADMIN"] as const)(
    "%s: lê o plano e o apontamento, mas NENHUMA ação de escrita renderiza",
    async (role) => {
      loginAs(role);

      render(<PlanDetail planId="plan-1" />);
  ...
  it.each(["MANAGER", "TECHNICIAN", "ADMIN"] as const)(
    "%s: sem apontamentos, vê o estado vazio SEM o atalho de criação",
    async (role) => {
      loginAs(role);
  ...
  it("PRODUCER: vê as 4 ações de escrita (não-regressão)", async () => {
    loginAs("PRODUCER");
